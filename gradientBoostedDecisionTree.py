...
#training
    ...
    trees = buildTreesGradBoosted(dfTrain, yAttrName);
    print("Training data:");
...
#predicting
    ...
    if np.array_equal(dfTest.columns.to_numpy(),dfTrain.columns.to_numpy())==False:
        ...
        exit(1);
    predVal = predictYBoost(trees, dfTrain, yAttrName, dfTest, verbosity=1);
    ...
    for k, tree in enumerate(trees):
        ...
        for i, node in enumerate(tree):
            if node["isLeaf"] != "No":
                ...
                bkColor = "white";
                df = node["dataframe"];
                dfIdx = df.index.values.tolist();
                prediction = 0;
                for j in dfIdx:
                    prediction += data[j][2];
                prediction = 1.0*prediction/len(dfIdx);
                if prediction <= 0.5:
                    bkColor = "red";
                else:
                    bkColor = "blue";
                ...
                rect=plt.Rectangle((xLower*xRange, yLower*yRange),\
                                   ...
                                   linewidth=0,\
                                   color=bkColor,\
                                   alpha=(aRatio-1.0)/(2.0*aRatio)*pow(aRatio,-k));
                ax.add_patch(rect);
    fileNameFig = fileNameTrain.replace("Train.txt", "Boosted.png");
...
###################################################################################
def getYPred(tree, columnNames, dataPoint, verbosity):
    if "str" in str(type(dataPoint)):
    ...
    while node["isLeaf"] == "No":
        ...
        if "str" in xLeftOptType:
            ...
            print("ERROR: getYPred(): unknown type for " + xLeftOpt + ".");
            exit(1);
        if selCond == True:
    ...
    try:
        ...
        exit(1);
def predictY(tree, dataframe, verbosity=0, yAttrOrigName=""):
    columnNames = dataframe.columns.tolist();
    yAttrName = tree[0]["yAttrName"];
    if yAttrOrigName == "":
        yAttrOrigName = yAttrName;
    yColIdx = columnNames.index(yAttrOrigName);
    columnNames[yColIdx] = yAttrName;
    data = dataframe.values.tolist();
    for i, dataPoint in enumerate(data):
        yPred = getYPred(tree, columnNames, dataPoint, verbosity);
        data[i][yColIdx] = yPred;
    return data;
###################################################################################
def buildTreesGradBoosted(dataframe, yAttrName, iterN=10):
    trees = [];
    columnNames = dataframe.columns.tolist();
    yColIdx = columnNames.index(yAttrName);
    data = dataframe.values.tolist();
    yData = listCol(data, yColIdx);
    yMean = [1.0*sum(yData)/max(1.0, len(yData))] * len(yData);
    yPred = yMean.copy();
    for i in range(BOOSTITERN):
        yRes = [(yD-yP) for yD, yP in zip(yData, yPred)];
        resAttrName = "res" + str(i);
        df = dataframe.drop(yAttrName, 1);
        df.insert(loc=yColIdx, column=resAttrName, value=yRes)
        columnNames = df.columns.tolist();
        yColIdx = columnNames.index(resAttrName);
        tree = buildTree(df, resAttrName);
        trees.append(tree);
        predRes = predictY(tree, df);
        yPredRes = listCol(predRes, yColIdx);
        yPred = [(yP + BOOSTRATE*yR) for yP, yR in zip(yPred, yPredRes)];
    return trees;
def predictYBoost(trees, dfTrain, yAttrName, dfTest, verbosity=0,iterN=BOOSTITERN):
    columnNames = dfTrain.columns.tolist();
    yColIdx = columnNames.index(yAttrName);
    dataTrain = dfTrain.values.tolist();
    yDataTrain = listCol(dataTrain, yColIdx);
    yMean = 1.0*sum(yDataTrain)/max(1.0, len(yDataTrain));

    dataTest = dfTrain.values.tolist();
    yDataTest = listCol(dataTest, yColIdx);
    yPred = [yMean] * len(yDataTest);
    for i, tree in enumerate(trees):
        if i < iterN:
            predRes = predictY(tree, dfTest, yAttrOrigName=yAttrName);
            yPredRes = listCol(predRes, yColIdx);
            yPred = [(yP + BOOSTRATE*yR) for yP, yR in zip(yPred, yPredRes)];
    for i, dataPoint in enumerate(dataTest):
        dataTest[i][yColIdx] = yPred[i]; 
    return dataTest;
###################################################################################
